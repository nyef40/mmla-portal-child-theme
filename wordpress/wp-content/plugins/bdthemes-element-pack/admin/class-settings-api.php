	...
	class ElementPack_Settings_API {
		...
		function do_settings_fields($page, $section) {
			...
			foreach ((array) $wp_settings_fields[$page][$section] as $field) {
				...
				if (!empty($field['args']['widget_type']) && 'pro' == $field['args']['widget_type'] && true !== element_pack_pro_activated()) {
					$data_type .= ' bdt-tooltip="Sorry, Element Pack is not activated!"';
				}
...
?>
			<div class="ep-option-item-inner">
				...
				if ($args['video_url']) :
				?>
					<a href="<?php echo esc_url($args['video_url']); ?>" target="_blank" class="ep-option-video" bdt-tooltip="View <?php echo esc_html($args['name']); ?> Video Tutorial">
				...
				?>
				<input type="<?php echo esc_attr($type); ?>" class="<?php echo esc_attr($class); ?>" id="<?php echo esc_attr($args['section']); ?>[<?php echo esc_attr($args['id']); ?>]" name="<?php echo esc_attr($args['section']); ?>[<?php echo esc_attr($args['id']); ?>]" value="<?php echo esc_attr($value); ?>" <?php echo wp_kses_post($placeholder); ?> />
				<?php
		...
		function show_navigation() {
			...
			/**
			 * Wincher Implementation
			 */
			$html .= sprintf('<li><a href="#%1$s" class="bdt-tab-item" id="bdt-%1$s" data-tab-index="%2$s">%3$s</a></li>', 'element_pack_seo_performance', $count, 'SEO Performance');
			$count = $count + 1;
			/**
			 * /Wincher Implementation
			 */
		...
		function element_pack_settings_save() {
			...
			if (isset($_POST[$moudle_id])) {
				error_log(print_r($_POST[$moudle_id], true));
												...
												<div bdt-dropdown="animation: bdt-animation-slide-top-small; duration: 300">
													<ul class="bdt-nav bdt-subnav-pill bdt-dropdown-nav ep-widget-filter ep-widget-content-type">
														...
														<?php if ($form['id'] == 'element_pack_active_modules') : ?>
															<li class="ep-widget-custom" bdt-filter-control="filter: [data-content-type*='custom']; group: data-widget-type">
																<a href="#">Custom</a>
															</li>
														<?php endif; ?>
														<li class="ep-widget-gallery" bdt-filter-control="filter: [data-content-type*='gallery']; group: data-widget-type">
															<a href="#">Gallery</a>
														</li>
														<li class="ep-widget-slider" bdt-filter-control="filter: [data-content-type*='slider']; group: data-widget-type">
															<a href="#">Slider</a>
														</li>
														<li class="ep-widget-carousel" bdt-filter-control="filter: [data-content-type*='carousel']; group: data-widget-type">
															<a href="#">Carousel</a>
														</li>
														<?php if ($form['id'] == 'element_pack_third_party_widget') : ?>
															<li class="ep-widget-acf" bdt-filter-control="filter: [data-content-type*='acf']; group: data-widget-type">
																<a href="#">ACF</a>
															</li>
															<li class="ep-widget-forms" bdt-filter-control="filter: [data-content-type*='forms']; group: data-widget-type">
																<a href="#">Forms</a>
															</li>
															<li class="ep-widget-ecommerce" bdt-filter-control="filter: [data-content-type*='ecommerce']; group: data-widget-type">
																<a href="#">eCommerce</a>
															</li>
														<?php endif; ?>
														<?php if ($form['id'] == 'element_pack_active_modules') : ?>
															<li class="ep-widget-template-builder" bdt-filter-control="filter: [data-content-type*='template-builder']; group: data-widget-type">
																<a href="#">Template Builder</a>
															</li>
														<?php endif; ?>
														<li class="ep-widget-others" bdt-filter-control="filter: [data-content-type*='others']; group: data-widget-type">
															<a href="#">Others</a>
														</li>
													</ul>
												</div>
											...
											<?php if ($form['id'] != 'element_pack_elementor_extend' or $form['id'] == 'element_pack_third_party_widget') : ?>
												...
												<div>
													<ul class="bdt-subnav bdt-subnav-pill ep-widget-filter ep-used-unused-widgets bdt-flex-inline">
														...
														<li class="ep-widget--" bdt-filter-control="filter: [data-content-type*='ep-unused']; group: data-content-type">
															<a href="#" bdt-tooltip="Don't need unused widget? Click on the Deactivate All button.">Unused
								...
								<div class="bdt-grid ">
									...
									<div class="bdt-width-auto@s ep-setting-save-btn">
										...
										<?php if (isset($this->settings_fields[$form['id']])) : ?>
											...
											<button class="bdt-button bdt-button-primary element-pack-settings-save-btn" type="submit">Save
												Settings</button>
