...
class Device_Slider extends Module_Base {
		...
		?>
				...
				<div class="bdt-device-slider-device">
					...
					<?php if ( $settings['device_type'] == 'custom' and 'yes' == $settings['show_custom_lens'] ) : ?>
						<img class="phone-lens" src="<?php echo esc_url(BDTEP_ASSETS_URL); ?>images/devices/phone-lens.svg"
							alt="Device Slider">
					<?php endif; ?>
	...
	protected function rendar_item_content( $content, $element_key ) {
		$settings = $this->get_settings_for_display();
		...
		<div
			...
			<?php if ( $content['title'] && ( 'yes' == $settings['show_title'] ) ) : ?>
				<div>
					<<?php echo esc_attr( Utils::get_valid_html_tag( $settings['title_tags'] ) ); ?> class="bdt-device-slider-title
	...
	public function render() {
		...
		foreach ( $settings['slides'] as $index => $slide ) :
			...
			<li class="bdt-slideshow-item elementor-repeater-item-<?php echo esc_attr( $slide['_id'] ); ?>" <?php echo ( $settings['global_link'] == 'yes' ) ? wp_kses_post($this->get_render_attribute_string( 'global-link' )) : ''; ?>>
				...
				<?php $this->rendar_item_content( $slide, 'link_' . $index ); ?>
			</li>
