...
class Post_Grid_Tab extends Module_Base {
	...
	public function render_content_image( $image_id, $size ) {
		...
		<div class="bdt-post-grid-tab-image">
			<div class="bdt-post-grid-tab-image-inner bdt-gt-mh bdt-cover-container">
				<?php
				if ( ! $image_src ) {
					printf( '<img src="%1$s" alt="%2$s" class="%2$s">', esc_url( $placeholder_image_src ), esc_html( get_the_title() ), esc_attr( $size ) );
				} else {
	...
	public function render_tab_image( $image_id ) {
		...
		?>
		<div class="bdt-post-grid-tab-thumbnail" title="<?php echo esc_html( get_the_title() ); ?>">
			<?php
			if ( ! $image_src ) {
	...
	public function render_title() {
		...
		<a href="<?php echo esc_url( get_the_permalink() ); ?>">
			<<?php echo esc_attr( Utils::get_valid_html_tag( $tag ) ); ?> class="bdt-post-grid-tab-item-title bdt-margin-remove">
				<?php the_title() ?>
	...
	public function render_readmore() {
		...
		<a href="<?php echo esc_url( get_permalink() ); ?>"
			class="bdt-post-grid-tab-readmore bdt-display-inline-block <?php echo esc_attr( $animation ); ?>">
	...
	public function render_header( $skin = 'default' ) {
		...
		?>
		<div <?php $this->print_render_attribute_string( 'post-grid-tab' ); ?>>
			<?php
	}
	...
	public function render_post_grid_tab_item( $post_id, $image_size = 'full' ) {
		...
		?>
			<div <?php $this->print_render_attribute_string( 'post-grid-tab-item' ) ?>>
	...
	public function render_post() {
		...
		?>
			...
			<dd>
				<?php $this->render_post_grid_tab_item( $post->ID, $settings['content_thumbnail_size'] ); ?>
			</dd>
			<?php
	}
}
