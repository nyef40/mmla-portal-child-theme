...
class Post_Grid extends Module_Base {
	...
	public function render_title() {
		...
		echo
			'<' . esc_attr( Utils::get_valid_html_tag( $settings['title_tags'] ) ) . ' class="bdt-post-grid-title">
	...
	public function render_author() {
		...
		echo
			'<span class="bdt-post-grid-author"><a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . wp_kses_post( get_the_author() ) . '</a></span>';
	}
	...
	public function render_date() {
		...
		if ( $settings['human_diff_time'] == 'yes' ) {
			$diff = element_pack_post_time_diff( ( $settings['human_diff_time_short'] == 'yes' ) ? 'short' : '' );
	...
	public function render_comments() {
		...
		echo
			'<div class="bdt-post-grid-comments bdt-inline-block"><span><i class="ep-icon-bubble" aria-hidden="true"></i> ' . wp_kses_post( get_comments_number() ) . '</span></div>';
	}
	...
	public function render_excerpt( $excerpt_length ) {
		...
		<div class="bdt-post-grid-excerpt">
			...
			if ( has_excerpt() ) {
				...
				echo wp_kses_post( element_pack_custom_excerpt( $excerpt_length, $strip_shortcode ) );
			}
			?>
		</div>
	...
	public function render_readmore() {
		...
		<a href="<?php echo esc_url( get_permalink() ); ?>"
			class="bdt-post-grid-readmore bdt-display-inline-block <?php echo esc_attr( $animation ); ?>">
	...
	private function loaded_post_ids() {
		$wp_query         = $this->get_query();
		$loaded_posts_ids = [];
		if ( $wp_query->have_posts() ) {
			while ( $wp_query->have_posts() ) {
				$wp_query->the_post();
				$loaded_posts_ids[] = get_the_ID();
			}
		}
		return $loaded_posts_ids;
	}
				...
				'bdt-post-grid' => [ 
					'data-settings' => [ 
						wp_json_encode( array_filter( [ 
							...
							'loadedPostsIds'                 => $this->loaded_post_ids(),
