...
class Post_Title extends Module_Base {
	...
	public function render() {
		...
		if ('yes' === $settings['bdt_post_link']) :
			printf('<a href="%3$s" class="ep-post-title-link"><%1$s><span class="ep-post-title-text">%2$s</span></%1$s></a>', esc_attr($title_tag), wp_kses_post($title_text), esc_url(get_permalink()));
		else :
			printf('<%1$s><span class="ep-post-title-text">%2$s</span></%1$s>', esc_attr($title_tag), wp_kses_post($title_text));
		endif;
	}
}
