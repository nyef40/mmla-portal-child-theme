...
class User_Login extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[ 
				...
				'type'      => Controls_Manager::TEXT,
				'default'   => esc_html__( 'Lost Password?', 'bdthemes-element-pack' ),
				'condition' => [ 
					'show_lost_password' => 'yes',
					'custom_labels'      => 'yes',
				],
		...
		$this->add_control(
			...
			[ 
				...
				'type'      => Controls_Manager::TEXT,
				'default'   => esc_html__( 'Remember Me', 'bdthemes-element-pack' ),
				'condition' => [ 
	...
	public function render_loop_custom_nav_list( $list ) {
		...
		<li class="bdt-user-login-custom-item">
			<a <?php
			$this->print_render_attribute_string( 'custom-nav-item' ); ?>>
				<?php
	...
	public function user_dropdown_menu() {
		...
		<div <?php
		$this->print_render_attribute_string( 'dropdown-settings' ); ?>>
			...
			<ul class="bdt-nav bdt-dropdown-nav">
				...
				if ( $settings['show_edit_profile'] ) : ?>
					<li>
						<a href="<?php echo esc_url( get_edit_user_link() ); ?>">
							<span class="bdt-ul-custom-nav-icon">
								<i class="ep-icon-edit fa-fw"></i>
							</span>
							<?php esc_html_e( 'Edit Profile', 'bdthemes-element-pack' ); ?>
						</a>
					</li>
					<?php
				endif; ?>
				...
				<li>
					<a href="<?php echo esc_url( wp_logout_url( $logout_url ) ); ?>" class="bdt-ul-logout-menu"><span
							class="bdt-ul-custom-nav-icon"><i class="ep-icon-lock fa-fw"></i></span>
				...
				<div class="bdt-user-login bdt-text-center">
					<ul class="bdt-list bdt-list-divider">
						...
						<li class="bdt-user-bio">
							<?php
							esc_html_e( 'Description:', 'bdthemes-element-pack' ) . ' '; ?>
							<?php
						...
						<li class="bdt-user-logged-out">
							...
							?>
							<a href="<?php
							echo esc_url( wp_logout_url( $logout_url ) ); ?>" class="bdt-button bdt-button-primary">
								<?php
	...
	public function social_login() {
		...
		if ( 'yes' == $settings['show_facebook_login'] or 'yes' == $settings['show_google_login'] ) : ?>
			<div class="bdt-width-1-1 bdt-width-1-2@s bdt-position-relative">
				<span <?php
				$this->print_render_attribute_string( 'separator' ); ?>>
					<div class="bdt-social-wrapper bdt-flex bdt-flex-middle">
						<div class="bdt-social-login">
							...
							if ( 'yes' == $settings['show_facebook_login'] ) : ?>
								<div <?php
								$this->print_render_attribute_string( 'facebook-login' ); ?>>
									<a href="javascript:void(0);" data-appid="<?php
									echo esc_attr( $gl_app_id ) ?>" class="fb_btn_link"><span class="bdt-facebook-icon"><i
												class="ep-icon-facebook fa-fw"></i></span>
										<?php
							...
							if ( ( ! $fb_secret or ! $fb_app_id ) && 'yes' == $settings['show_facebook_login'] ) :
								...
								<div class="bdt-alert-warning" data-bdt-alert>
									...
									<p>
										<?php
										echo sprintf( esc_html__( 'Ops! %1s Missing. Please add them from: Element Pack Settings > API Settings > Social Login Access. ', 'bdthemes-element-pack' ), esc_html( $fb_message ) ); ?>
									</p>
								</div>
							...
							if ( 'yes' == $settings['show_google_login'] ) : ?>
								<div class="bdt-google">
									...
									echo esc_attr( $gl_app_id ) ?>" id="google_btn_link"><span class="bdt-google-icon"><i
												class="ep-icon-google fa-fw"></i></span>
										<?php
										echo esc_html( 'Google' ); ?>
									</a>
								</div>
								<?php
							endif; ?>
							...
							if ( ! $gl_app_id && 'yes' == $settings['show_google_login'] ) :
								...
								<div class="bdt-alert-warning" data-bdt-alert>
									...
									<p>
										<?php
										echo sprintf( esc_html__( 'Ops! %1s Missing. Please add them from: Element Pack Settings > API Settings > Social Login Access. ', 'bdthemes-element-pack' ), esc_html( $google_app_message ) ); ?>
									</p>
								</div>
	...
	public function user_login_form() {
		...
		echo esc_attr( $id ); ?>" <?php
			$this->print_render_attribute_string( 'login-form-wrapper' ); ?> method="post">
			...
			<div <?php
			$this->print_render_attribute_string( 'field-group' ); ?>>
				<?php
				if ( $settings['show_labels'] ) {
					?>
					<label <?php
					$this->print_render_attribute_string( 'user_label' ); ?>>
						...
						if ( 'yes' == $settings['custom_labels'] ) {
							echo wp_kses_post( $settings['user_label'] );
						} else {
				...
				echo '<input ' . wp_kses_post( $this->get_render_attribute_string( 'user_input' ) ) . ' required>';
				echo '</div>';
			...
			<div <?php
			$this->print_render_attribute_string( 'submit-group' ); ?>>
				...
				$this->print_render_attribute_string( 'button' ); ?>>
					...
					if ( ! empty ( $settings['button_text'] ) ) : ?>
						<span>
							<?php
							echo wp_kses( $settings['button_text'], element_pack_allow_tags( 'title' ) ); ?>
						</span>
						<?php
					endif; ?>
				</button>
