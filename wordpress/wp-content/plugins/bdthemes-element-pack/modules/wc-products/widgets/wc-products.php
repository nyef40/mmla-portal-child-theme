...
class WC_Products extends Module_Base {
	...
	public function render_quick_view( $product_id ) {
		...
		if ( 'yes' == $settings['show_quick_view'] ) : ?>
			<div class="bdt-quick-view <?php echo esc_attr( $settings['quick_view_hide_mobile'] ? 'bdt-visible@s' : '' ) ?>">
				...
				<input type="hidden" class="bdt_modal_spinner_message"
					value="<?php echo esc_html__( 'Please wait...', 'bdthemes-element-pack' ); ?>" />
				<a href="javascript:void(0)" data-id="<?php echo absint( $product_id ); ?>"
					data-bdt-tooltip="title: <?php echo esc_html__( 'Quick View', 'bdthemes-element-pack' ); ?>; pos: left;">
	...
	public function render_filter_menu() {
		...
		<div class="bdt-ep-grid-filters-wrapper" id="bdt-products-<?php echo esc_attr( $this->get_id() ); ?>" <?php $this->print_render_attribute_string( 'portfolio-gallery-hash-data' ); ?>>
			...
			<div data-bdt-dropdown="mode: click;"
				...
				<ul class="bdt-nav bdt-dropdown-nav">
					<li class="bdt-ep-grid-filter bdt-active" data-bdt-filter-control><a href="#">
							<?php esc_html_e( 'All Products', 'bdthemes-element-pack' ); ?>
						</a></li>
	...
	public function render_loop_item() {
		...
		if ( $wp_query->have_posts() ) {
			...
			?>
			<div <?php $this->print_render_attribute_string( 'wc-products-wrapper' ); ?>>
				<?php
				...
				while ( $wp_query->have_posts() ) :
					...
					<div <?php $this->print_render_attribute_string( 'wc-product' ); ?>>
						<div class="bdt-wc-product-inner">
							...
							<?php if ( $settings['show_badge'] and ! $product->is_in_stock() ) : ?>
								<div class="bdt-badge bdt-position-top-left bdt-position-small">
									<?php echo wp_kses_post(apply_filters( 'woocommerce_product_is_in_stock', '<span class="bdt-onsale">' . esc_html__( 'Out of Stock!', 'woocommerce' ) . '</span>', $post, $product )); ?>
								</div>
							<?php elseif ( $settings['show_badge'] and $product->is_on_sale() ) : ?>
								<div class="bdt-badge bdt-position-top-left bdt-position-small">
									<?php echo wp_kses_post(apply_filters('woocommerce_product_sale_flash', '<span class="bdt-onsale">' . esc_html__( 'Sale!', 'woocommerce' ) . '</span>', $post, $product )); ?>
								</div>
							<?php endif; ?>
							...
							<div class="bdt-wc-product-desc">
								<?php if ( 'yes' == $settings['show_title'] ) : ?>
									<<?php echo esc_attr( Utils::get_valid_html_tag( $settings['title_tags'] ) ); ?>
										<?php $this->print_render_attribute_string( 'bdt-wc-product-title' ); ?>>
