...
function qev_remaining_credits() {
    ...
    $results = wp_remote_get($url, array('headers' => array( 'user-agent' => 'quickemailverification-wp-plugin/v1.10.0')));
...
function qev_single($emailAddress,$api_key, $qev_options)
{
	try {
		...
		// Used WordPress HTTP API method to get the result from QuickEmailVerification API.
                $results = wp_remote_get($url, array('headers' => array( 'user-agent' => 'quickemailverification-wp-plugin-'.$qev_options['form_name'].'/v1.10.0'),'timeout'=> 60));
