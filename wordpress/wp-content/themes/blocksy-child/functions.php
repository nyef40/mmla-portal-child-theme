...
/**
 * Process the validation_url smart tag with HTML button formatting
 */
function wpforms_smart_tag_process_validation_url($content, $tag, $form_data, $fields, $entry_id) {
    ...
    if ($validation_url) {
        ...
        // Return complete HTML email template instead of just URL
...
function last_chance_email_modification($args) {
    ...
    if (!empty($to)) {
        ...
        if ($entry_id) {
            ...
            if ($validation_url) {
                ...
                error_log("Last chance replaced with HTML template", 3, WP_CONTENT_DIR . '/debug.log');
            }
        } else {
    ...
    if (strpos($args['message'], '{validation_url}') !== false) {
        ...
        if ($submission) {
            ...
            $html_template = get_provider_email_template('Doctor', $validation_url);
            $args['message'] = str_replace('{validation_url}', $html_template, $args['message']);
            error_log("Last resort replaced with HTML template", 3, WP_CONTENT_DIR . '/debug.log');
        }
    }
