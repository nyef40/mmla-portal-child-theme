...
#
# Usage (on server, from theme root):
#   cd ~/public_html/wp-content/themes/blocksy-child
#   chmod +x test-auth.sh
#   PORTAL_USER=mmla2024 PORTAL_PASS=tiger2025 ./test-auth.sh
#
# Optional env: BASE_URL (default https://mobilemedicalla.com), PORTAL_USER, PORTAL_PASS
...
# Login page may load nonce via AJAX; get fresh nonce from endpoint
if [ -z "$NONCE" ] || [ ${#NONCE} -lt 8 ]; then
  echo "[test:auth] No nonce in HTML, fetching from portal_login_fresh_nonce..."
  NONCE_RESP=$(curl -sS -L -X POST "$AJAX_URL" \
    -H "Content-Type: application/x-www-form-urlencoded" \
    -d "action=portal_login_fresh_nonce")
...
if echo "$RESP" | grep -q 'Security check failed'; then
  echo "[test:auth] FAIL: Security check failed (nonce rejected)."
  exit 1
fi
